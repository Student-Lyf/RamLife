...
class Day {
	...
	static List<List<Day?>> getCalendar(List<List<Json?>> year) => [
		for (final List<Json?> month in year) [
			for (final Json? day in month)
				if (day == null) null
	...
	factory Day.fromJson(Json json) {
		final String scheduleName = json ["schedule"];
		final schedule = Schedule.schedules.firstWhere(
	...
	int? getCurrentPeriod() {
		final time = Time.fromDateTime(DateTime.now());
		for (var index = 0; index < schedule.periods.length; index++) {
			final range = schedule.periods [index].time;
